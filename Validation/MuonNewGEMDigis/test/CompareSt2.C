...
void CompareSt2()
{
  ...
  TString total     = tDir+"/NoPrompt/strip_dg_R_st2_NoPrompt";
  TString electrons = tDir+"/NoPrompt/strip_dg_R_st2_NoPrompt_e";
  TString neutrons  = tDir+"/NoPrompt/strip_dg_R_st2_NoPrompt_n";
  TString gammas    = tDir+"/NoPrompt/strip_dg_R_st2_NoPrompt_g";
  ...
  leg_hits->SetHeader("GEM St2 Neutron Bkg Hits");
  ...
  TF1 *f1 = new TF1("myfunc1",myfunction1,130,350,6);
  f1->SetParameters(21583.2, -476.59, 4.24037, -0.0185558, 3.97809e-05, -3.34575e-08);
  // f1->SetParNames("constant","coefficient");
  f1->Draw();

  
  TF1 *f2 = new TF1("myfunc2",myfunction2,130,350,2);
  f2->SetParameters(9.74156e+02, -1.18398e-02);
  ...
  */
  ...
  leg->SetFillColor(0);
  leg->SetHeader("GEM St 2 Neutron Bkg Param");
  ...
  c_all->SetGrid(1,1);
  c_all->SetLogy();
  c_all->SetTitle("GEM St 2 Neutron Background Parameterization");
  ...
  leg2->SetFillColor(0);
  leg2->SetHeader("GEM St 2 Neutron Bkg Param and Hits");
...
void prepareHisto (TH1F* hist, Int_t rebin, Double_t weight, Color_t color){
  ...
  hist->GetXaxis()->SetRangeUser(130,350);

...
GEMPreRecoSegmentBuilder::GEMPreRecoSegmentBuilder(const edm::ParameterSet& ps) : geom_(0) {
  
  // Algo name
  algoName = ps.getParameter<std::string>("algo_name");
  
  edm::LogVerbatim("GEMPreRecoSegmentBuilder")<< "GEMPreRecoSegmentBuilder algorithm name: " << algoName;
...
GEMPreRecoSegmentBuilder::~GEMPreRecoSegmentBuilder() {
  delete algo;
}


void GEMPreRecoSegmentBuilder::build(const GEMRecHitCollection* recHits, GEMSegmentCollection& ocT) {
  	
  edm::LogVerbatim("GEMPreRecoSegmentBuilder")<< "[GEMPreRecoSegmentBuilder::build] Total number of rechits in this event: " << recHits->size();
  ...
  for(auto enIt=ensembleRH.begin(); enIt != ensembleRH.end(); ++enIt) {
    ...
    std::vector<const GEMRecHit*> gemRecHitsPreReco;
    ...
    for(auto rechit = enIt->second.begin(); rechit != enIt->second.end(); ++rechit) {
      gemRecHitsPreReco.push_back(*rechit);
      ens[(*rechit)->gemId()]=geom_->etaPartition((*rechit)->gemId());
    }    

    #ifdef EDM_ML_DEBUG // have lines below only compiled when in debug mode
    edm::LogVerbatim("GEMPreRecoSegmentBuilder") << "[GEMPreRecoSegmentBuilder::build] -----------------------------------------------------------------------------"; 
    edm::LogVerbatim("GEMPreRecoSegmentBuilder") << "[GEMPreRecoSegmentBuilder::build] found " << gemRecHitsPreReco.size() << " rechits in GEM Super Chamber " << chamber->id()<<" ::"; 
    for (auto rh=gemRecHitsPreReco.begin(); rh!=gemRecHitsPreReco.end(); ++rh){
      auto gemid = (*rh)->gemId();
      ...
      edm::LogVerbatim("GEMPreRecoSegmentBuilder") << "[RecHit :: Loc x = "<<std::showpos<<std::setw(9)<<rhLP.x() /*<<" Loc y = "<<std::showpos<<std::setw(9)<<rhLP.y()*/
    ...
    edm::LogVerbatim("GEMPreRecoSegmentBuilder") << "[GEMPreRecoSegmentBuilder::build] run the segment reconstruction algorithm now";
    // given the superchamber select the appropriate algo... and run it
    std::vector<GEMSegment> segv = algo->run(ensemble, gemRecHitsPreReco);
    GEMDetId mid(enIt->first);
    edm::LogVerbatim("GEMPreRecoSegmentBuilder") << "[GEMPreRecoSegmentBuilder::build] found " << segv.size() << " segments in GEM Super Chamber " << mid;
    edm::LogVerbatim("GEMPreRecoSegmentBuilder") << "[GEMPreRecoSegmentBuilder::build] -----------------------------------------------------------------------------"; 
    // Add the segments to master collection
    ocT.put(mid, segv.begin(), segv.end());
  }
}

void GEMPreRecoSegmentBuilder::setGeometry(const GEMGeometry* geom) {
  geom_ = geom;
}

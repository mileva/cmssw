...
private:
  ...
  std::string myInputFile15_;
  std::string myInputFile11_;
...
//    uint32_t id;
    ...
    std::map<double, double> pairHvRate15;
    std::map<double, double> pairHvRate11;
  };
...
MyNoiseScan::MyNoiseScan(const edm::ParameterSet& iConfig)
{
  ...
  myInputFile15_ = iConfig.getParameter < std::string > ("myInputFile15");
  myInputFile11_ = iConfig.getParameter < std::string > ("myInputFile11");
  ...
  doAll_= iConfig.getParameter<bool>("doAll");
  doAllPics_= iConfig.getParameter<bool>("doAllPics");
  doAllMacros_= iConfig.getParameter<bool>("doAllMacros");
...
//  double stripArea = 0.0;
  ...
  std::map<double, double> tempMap15;
  std::map<double, double> tempMap11;
  string graphName;   //for doing scan per every roll and store them as pics
  string fileName;
  string titleFoGraph;
  std::map<double, double>::iterator itscan;
  ...
  double rate, rate15, rate11;
  double hvpoint, hvpoint15, hvpoint11;
  string rollName, rollName15, rollName11;
  fstream f_input, f_input15, f_input11;
...
//fill for 2015
        f_input15.open(myInputFile15_, ios::in);
        for (; f_input15 >> rollName15 >> hvpoint15 >> rate15;)
        {
#ifdef debug_
          std::cout << rollName15 << "\t" << hvpoint15 << "\t" << rate15 << std::endl;
#endif
          if (rollName.compare(rollName15) == 0)
          {
            tempMap15.insert(std::pair<double, double>(hvpoint15, rate15));
          }
        }
        f_input15.close();
//fill for 2015 end
        //fill for 2011
        f_input11.open(myInputFile11_, ios::in);
        for (; f_input11 >> rollName11 >> hvpoint11 >> rate11;)
        {
#ifdef debug_
          std::cout << rollName11 << "\t" << hvpoint11 << "\t" << rate11 << std::endl;
#endif
          if (rollName.compare(rollName11) == 0)
          {
            tempMap11.insert(std::pair<double, double>(hvpoint11, rate11));
          }
        }
        f_input11.close();
        //fill for 2011 end

        subObj.pairHvRate15 = tempMap15;
        subObj.pairHvRate11 = tempMap11;
    ...
    f_input.close();
    if (debug_)
      std::cout << "sub.size() = " << sub.size() << "\t" << justcounter << std::endl;

    for (unsigned int i = 0; i < sub.size(); i++)
    {
      if (debug_)
        std::cout << sub[i].name << "\t" << sub[i].id << "\tmap size = " << (sub[i].pairHvRate).size() << "\t" << (sub[i].pairHvRate)[8800] << std::endl;
...
//Fill 2D histos rolls vs sector
      ...
      if ((sub[i].id).region() == -1)
      {
        if ((sub[i].id).station() == 1)
        {
          Diskm1RollSector->Fill(rpcsrv.segment(), Y, rate2D);
        }
        else if ((sub[i].id).station() == 2)
        {
          Diskm2RollSector->Fill(rpcsrv.segment(), Y, rate2D);
        }
        else if ((sub[i].id).station() == 3)
        {
          Diskm3RollSector->Fill(rpcsrv.segment(), Y, rate2D);
        }
        else if ((sub[i].id).station() == 4)
        {
          Diskm4RollSector->Fill(rpcsrv.segment(), Y, rate2D);
        }
        else
          std::cout << " no match for " << sub[i].id << std::endl;
      }
      if ((sub[i].id).region() == 1)
      {
        if ((sub[i].id).station() == 1)
        {
          Diskp1RollSector->Fill(rpcsrv.segment(), Y, rate2D);
        }
        else if ((sub[i].id).station() == 2)
        {
          Diskp2RollSector->Fill(rpcsrv.segment(), Y, rate2D);
        }
        else if ((sub[i].id).station() == 3)
        {
          Diskp3RollSector->Fill(rpcsrv.segment(), Y, rate2D);
        }
        else if ((sub[i].id).station() == 4)
        {
          Diskp4RollSector->Fill(rpcsrv.segment(), Y, rate2D);
        }
        else
          std::cout << " no match for " << sub[i].id << std::endl;
      }
      if ((sub[i].id).region() == 0)
      {
        ...
        {
          if (debug_)
            std::cout << "Wheel " << (sub[i].id).ring() << "\tid = " << (sub[i].id) << "\t"
                << rollB(rpcsrv.shortname(), rollNamesExter) << "\t" << rpcsrv.shortname() << std::endl;
          if ((sub[i].id).ring() == 2)
            Wheel2RollSector->Fill((sub[i].id).sector(), rollB(rpcsrv.shortname(), rollNamesExter), rate2D);
          else if ((sub[i].id).ring() == -2)
            Wheelm2RollSector->Fill((sub[i].id).sector(), rollB(rpcsrv.shortname(), rollNamesExter), rate2D);
          else if ((sub[i].id).ring() == -1)
            Wheelm1RollSector->Fill((sub[i].id).sector(), rollB(rpcsrv.shortname(), rollNamesInter), rate2D);
          else if ((sub[i].id).ring() == 0)
            Wheel0RollSector->Fill((sub[i].id).sector(), rollB(rpcsrv.shortname(), rollNamesInter), rate2D);
          else if ((sub[i].id).ring() == 1)
            Wheel1RollSector->Fill((sub[i].id).sector(), rollB(rpcsrv.shortname(), rollNamesInter), rate2D);
          else
            std::cout << " no match for " << sub[i].id << std::endl;
        }
      }
//do scan plots per every roll
      if (doAll_)
      {
        ostringstream oss;
        oss << sub[i].name;
        fileName = oss.str().c_str();
        titleFoGraph = "Noise scan for " + fileName;
        if (debug_)
          std::cout << sub[i].name << std::endl;
        if (doAllPics_) graphName = "pics/" + fileName + ".png";
        if (doAllMacros_) graphName = "macros/" + fileName + ".C";
        int mycount = 1;
        int mycount15 = 1;
        int mycount11 = 1;
        int kk;
        std::vector<double> tempVi;

        if ((sub[i].id).region() == 0) kk = 9;
        else if(abs((sub[i].id).region()) == 1) kk = 10;
        else std::cout << "wrong Id region in doAll, region = "<< (sub[i].id).region() << std::endl;
        TGraphErrors* myGr = new TGraphErrors(kk);
        TGraphErrors* myGr15 = new TGraphErrors(kk);
        TGraphErrors* myGr11 = new TGraphErrors(kk);
        for (itscan = (sub[i].pairHvRate).begin(); itscan != (sub[i].pairHvRate).end(); ++itscan)
        {
          myGr->SetPoint(mycount, itscan->first, itscan->second);
          tempVi.push_back(itscan->second);
          mycount++;
        }
        for (itscan = (sub[i].pairHvRate15).begin(); itscan != (sub[i].pairHvRate15).end(); ++itscan)
        {
          myGr15->SetPoint(mycount15, itscan->first, itscan->second);
          tempVi.push_back(itscan->second);
          mycount15++;
        }
        for (itscan = (sub[i].pairHvRate11).begin(); itscan != (sub[i].pairHvRate11).end(); ++itscan)
        {
          myGr11->SetPoint(mycount11, itscan->first, itscan->second);
          tempVi.push_back(itscan->second);
          mycount11++;
        }

        double max = *max_element(tempVi.begin(), tempVi.end());
        if(debug_) cout<<"Max value: "<<max<<endl;

        c2->cd();
        myGr->SetMarkerStyle(22);
        myGr->SetMarkerSize(1.5);
        myGr->SetMarkerColor(kRed + 2);
        myGr->SetTitle(titleFoGraph.c_str());
        myGr->GetYaxis()->SetTitle("Rate [Hz/cm^{2}]");
        myGr->GetYaxis()->SetTitleSize(0.05);
        myGr->GetYaxis()->SetTitleOffset(0.8);
        myGr->GetXaxis()->SetTitle("HV_{Eff} [V]");
        myGr->GetXaxis()->SetTitleOffset(1);
        myGr->GetXaxis()->SetTitleSize(0.04);
        myGr->GetXaxis()->SetRangeUser(8750, 9850);
        myGr->GetYaxis()->SetRangeUser(0, max+0.05);
        myGr->DrawClone("AZP");

        myGr15->SetMarkerStyle(20);
        myGr15->SetMarkerSize(1.5);
        myGr15->SetMarkerColor(kBlue + 2);
        myGr15->SetTitle(titleFoGraph.c_str());
        myGr15->GetYaxis()->SetTitle("Rate [Hz/cm^{2}]");
        myGr15->GetYaxis()->SetTitleSize(0.05);
        myGr15->GetYaxis()->SetTitleOffset(0.8);
        myGr15->GetXaxis()->SetTitle("HV_{Eff} [V]");
        myGr15->GetXaxis()->SetTitleOffset(1);
        myGr15->GetXaxis()->SetTitleSize(0.04);
        myGr15->GetXaxis()->SetRangeUser(8750, 9850);
        myGr15->GetYaxis()->SetRangeUser(0, max+0.05);
        myGr15->DrawClone("PE");

        myGr11->SetMarkerStyle(21);
        myGr11->SetMarkerSize(1.5);
        myGr11->SetMarkerColor(kGreen + 2);
        myGr11->SetTitle(titleFoGraph.c_str());
        myGr11->GetYaxis()->SetTitle("Rate [Hz/cm^{2}]");
        myGr11->GetYaxis()->SetTitleSize(0.05);
        myGr11->GetYaxis()->SetTitleOffset(0.8);
        myGr11->GetXaxis()->SetTitle("HV_{Eff} [V]");
        myGr11->GetXaxis()->SetTitleOffset(1);
        myGr11->GetXaxis()->SetTitleSize(0.04);
        myGr11->GetXaxis()->SetRangeUser(8750, 9850);
        myGr15->GetYaxis()->SetRangeUser(0, max+0.05);
        myGr11->DrawClone("PE");
        
        TLegend *leg = new TLegend(0.12,0.6,0.29,0.39);
        leg->SetBorderSize(1);
        leg->SetTextSize(0.0338461);
        leg->SetLineColor(0);
        leg->SetLineStyle(1);
        leg->SetLineWidth(1);
        leg->SetFillColor(0);
        ...
        TLegendEntry *entry=leg->AddEntry("myGr","NoiseScan 2018","p");
        ...
        entry=leg->AddEntry("myGr15","Noise Scan 2015","p");
        entry->SetLineColor(1);
        entry->SetLineStyle(1);
        entry->SetLineWidth(1);
        ...
        entry->SetMarkerSize(1.5);
        entry->SetTextFont(42);
        entry=leg->AddEntry("myGr11","Noise Scan 2011","p");
        entry->SetLineColor(1);
        entry->SetLineStyle(1);
        entry->SetLineWidth(1);
  ...
  if (doNoiseNoMagnet_)
  {
    ...
    if (debug_)
      std::cout << "sub.size() = " << subNoMagnet.size() << std::endl;
    for (unsigned int i = 0; i < subNoMagnet.size(); i++)
    {
      if (debug_)
        std::cout << subNoMagnet[i].name << "\t" << subNoMagnet[i].id << "\tmap size = " << (subNoMagnet[i].pairHvRate).size() << "\t" << (subNoMagnet[i].pairHvRate)[8800] << std::endl;
...
//for 2D plots rolls vs sector
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    if (i == 0)
      Diskm4RollSector = rollVsSectorE.make < TH2F > ("Diskm4RollSector", titleLabel.str().c_str(), 36, 0.5, 36.5, 6, 0.5, 6.5);
    if (i == 1)
      Diskm3RollSector = rollVsSectorE.make < TH2F > ("Diskm3RollSector", titleLabel.str().c_str(), 36, 0.5, 36.5, 6, 0.5, 6.5);
    if (i == 2)
      Diskm2RollSector = rollVsSectorE.make < TH2F > ("Diskm2RollSector", titleLabel.str().c_str(), 36, 0.5, 36.5, 6, 0.5, 6.5);
    if (i == 3)
      Diskm1RollSector = rollVsSectorE.make < TH2F > ("Diskm1RollSector", titleLabel.str().c_str(), 36, 0.5, 36.5, 6, 0.5, 6.5);
  }
  for (int i = 0; i < 4; i++)
  {
    titleLabel.str("");
    ...
    if (i == 0)
      Diskp1RollSector = rollVsSectorE.make < TH2F > ("Diskp1RollSector", titleLabel.str().c_str(), 36, 0.5, 36.5, 6, 0.5, 6.5);
    if (i == 1)
      Diskp2RollSector = rollVsSectorE.make < TH2F > ("Diskp2RollSector", titleLabel.str().c_str(), 36, 0.5, 36.5, 6, 0.5, 6.5);
    if (i == 2)
      Diskp3RollSector = rollVsSectorE.make < TH2F > ("Diskp3RollSector", titleLabel.str().c_str(), 36, 0.5, 36.5, 6, 0.5, 6.5);
    if (i == 3)
      Diskp4RollSector = rollVsSectorE.make < TH2F > ("Diskp4RollSector", titleLabel.str().c_str(), 36, 0.5, 36.5, 6, 0.5, 6.5);
  }
  ...
  for (int i = 0; i < 5; i++)
  {
    ...
    if (i == 0)
      Wheelm2RollSector = rollVsSectorB.make < TH2F > ("Wheelm2RollSector", titleLabel.str().c_str(), 12, 0.5, 12.5, 22, 0.5, 22.5);
    if (i == 1)
      Wheelm1RollSector = rollVsSectorB.make < TH2F > ("Wheelm1RollSector", titleLabel.str().c_str(), 12, 0.5, 12.5, 22, 0.5, 22.5);
    if (i == 2)
      Wheel0RollSector = rollVsSectorB.make < TH2F > ("Wheel0RollSector", titleLabel.str().c_str(), 12, 0.5, 12.5, 22, 0.5, 22.5);
    if (i == 3)
      Wheel1RollSector = rollVsSectorB.make < TH2F > ("Wheel1RollSector", titleLabel.str().c_str(), 12, 0.5, 12.5, 22, 0.5, 22.5);
    if (i == 4)
      Wheel2RollSector = rollVsSectorB.make < TH2F > ("Wheel2RollSector", titleLabel.str().c_str(), 12, 0.5, 12.5, 22, 0.5, 22.5);
  }
  for (int i = 1; i <= 12; i++)
  {
    binLabel.str("");
